...
function errorHandler(req, res, code, mess) {
    res.writeHead(500, 'Hi', { 'Content-Type': 'application/json; charset=utf-8'});
    res.end(`{"error": "${code}", "message": "${mess}"}`)
}

function modifiedReadFile() {
    fs.accessSync(listFilePath, fs.constants.F_OK);
    console.log(`File ${listFilePath} accessed`);
        ...
        switch(true) {
                ...
                req.on('end', () => {
                    ...
                    if(!flag) {
                        ...
                        fs.writeFile(listFilePath, JSON.stringify(fetchedFromFile), (e) => {
                            if (e) {
                                console.log('Error occurred during writeFile');
            ...
            case path === '/backup': {
                ...
                fs.copyFile(listFilePath, `./backup/${date}_StudentList.json`, (e) => {
                    if (e) {
                        console.log('error');
                        errorHandler(req, res, e.code, e.messacge);
                    }
                    else {
            ...
            case path === '/': {
                ...
                req.on('end', () => {
                    ...
                    if(flag) {
                        fs.writeFile(listFilePath, JSON.stringify(fetchedFromFile), (e) => {
                            if (e) {
                                console.log('Error is occurred during writeFile');
                                errorHandler(req, res, e.code, e.message);
                            } else {
                        ...
                        if (files[i].match(/\d{8}/)[0] < Number(path.match(/\d+/))) {
                            ...
                            fs.unlink(`./backup/${files[i]}`, (e) => {
                                if (e) {
                                    ...
                                    res.end('Outdated logfiles deleted');
                                }
                            });
                        }
                    }
